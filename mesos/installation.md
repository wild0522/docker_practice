...
ZooKeepr 是一個分佈式應用的協調工具，用來管理多個 Master 節點的選舉和冗余，監聽在 2181 端口。
...
第一個端口負責從節點連接到主節點的；第二個端口負責主節點的選舉通信。
...
* /etc/mesos-master：主節點的配置，等價於啟動 mesos-master 時候的默認選項；
* /etc/mesos-slave：從節點的配置，等價於啟動 mesos-master 時候的默認選項。
...
同時可以通過瀏覽器訪問 8080 端口，看到 marathon 的管理界面。
